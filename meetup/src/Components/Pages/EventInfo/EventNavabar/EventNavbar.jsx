  ...
  const handleClick = () => {
    ...
    if (!isGoing) {
      ...
      const attendeeData = {
        name,
        img:
          'https://image.shutterstock.com/z/stock-photo-side-view-of-young-ethnic-man-with-earphones-and-short-hair-using-smartphone-while-leaning-on-1687201528.jpg',
      };
  ...
  return (
    <div className={Styles.eventInfo__navabar_container}>
      ...
      <div className={Styles.event_goingDiv}>
        ...
        <Button
          className={Styles.eventInfo__navbar_attend_online_button}
          onClick={handleClick}
        >
          {isGoing ? "Don't wanna go" : 'Attend Online'}
        </Button>
        <p style={{ marginLeft: '30px', color: 'grey', marginTop: '10px' }}>
          {isGoing ? 'Edit RSVP' : ''}
        </p>
        <Modal
          className={Styles.eventInfo__navbar_modal}
          centered
        ...
        >
          ...
          >
            <img
              className={Styles.eventInfo__host_image}
              src={img}
              alt='attende'
            />
            <br />
          ...
          <p className={Styles.modal_para}>
            If anything changes just update your response.
          </p>
          <p className={Styles.modal_para}>
            It helps host to prepare and its polite :) .
          </p>
